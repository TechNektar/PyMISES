...
def run_rae2822_case6_validation(export_dir=None, compare_with_exp=True):
    ...
    airfoil_indices = [i for i in range(ni)]
    ...
    farfield_indices = [i + (nj-1) * ni for i in range(ni)]
    ...
    if compare_with_exp:
        ...
        if 'displacement_thickness' in final_solution and 'momentum_thickness' in final_solution:
            fig_bl, ax_bl = plt.subplots(figsize=(10, 6))

            # Plot numerical results
            delta_star_upper = final_solution['displacement_thickness'][upper_surface][upper_sort]
            theta_upper = final_solution['momentum_thickness'][upper_surface][upper_sort]
    ...
    if export_dir is not None:
        ...
        pd.DataFrame({
            'x/c': np.concatenate([x_upper, x_lower]),
            'y/c': np.concatenate([y_coords[upper_surface][upper_sort], y_coords[lower_surface][lower_sort]]),
            'cp': np.concatenate([cp_upper, cp_lower])
        }).to_csv(export_path / 'rae2822_case6_pressure.csv', index=False)

        # Export boundary layer data
        if 'displacement_thickness' in final_solution and 'momentum_thickness' in final_solution:
            bl_data = {
                'x/c': x_upper,
                'delta_star': delta_star_upper,
                'theta': theta_upper,
                'H': H_upper,
                'cf': final_solution.get('skin_friction', np.zeros_like(delta_star_upper))[upper_surface][upper_sort]
            }
