...
def run_cascade_analysis(blade_file, inlet_angle=30.0, outlet_angle=None, mach_in=0.5, 
    ...
    grid_gen = GridGenerator(blade, grid_type='c-grid')
    grid = grid_gen.generate_cascade_grid(
        pitch=pitch,
        n_normal=41,
        n_tangential=61,
        n_wake=25,
        downstream_length=2.0
    )
    
    # 3. Set up boundary conditions
    
    # Wall boundary condition on blade surface
    blade_indices = grid.get_boundary_indices('blade')
    wall_bc = InviscidWallBC(blade_indices, normal_direction='inner')
    
    # Periodicity boundary condition
    upper_indices = grid.get_boundary_indices('upper_periodic')
    lower_indices = grid.get_boundary_indices('lower_periodic')
    periodicity_bc = PeriodicityBC(upper_indices, lower_indices, pitch)
    
    # Inlet boundary condition
    inlet_indices = grid.get_boundary_indices('inlet')
    ...
    outlet_indices = grid.get_boundary_indices('outlet')
